...
export const Contact: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const emailSub = fromEvent(emailRef.current, 'input')
      ...
      .subscribe((val) => {
        ...
        setErrors((prev) => ({
          ...prev,
          email: val.trim() === '' ? '' : (!emailRegex.test(val) ? t('contact.form.error.email') : ''),
        }));
      });
    ...
    const messageSub = fromEvent(messageRef.current, 'input')
      ...
      .subscribe((val) => {
        setErrors((prev) => ({
          ...prev,
          message: val.trim() === '' ? '' : (val.trim().length < 10 ? t('contact.form.error.message') : ''),
        }));
      });
  ...
  return (
    <section id="contact" className="py-24 px-6 max-w-3xl mx-auto border-t border-glass-border/40">
      ...
      <form onSubmit={handleSubmit} className="p-6 md:p-8 rounded-2xl glass-panel bg-glass-bg flex flex-col gap-6">
        {/* Name field */}
